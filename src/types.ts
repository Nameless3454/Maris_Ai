...
export interface AppSettings {
  ...
  data: {
    dataSource: 'DEMO / SIMULATED' | 'LIVE SATELLITE FEED';
    aisSync: 'SIMULATED' | 'LIVE AIS STREAM';
    satelliteSync: 'SIMULATED' | 'LIVE SENSORS (SENTINEL-1)';
  };
}
...
export type AnalysisViewMode = 'original' | 'detected' | 'mask' | 'comparison';

export interface SimulatedSpillResult {
  confidence: number;
  areaKm2: number;
  estimatedAge: string;
  severity: 'HIGH' | 'CRITICAL' | 'MEDIUM';
  spillCenter: { x: number; y: number };
