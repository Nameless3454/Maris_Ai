...
import { Incident, NavItem, AppNotification, Vessel, MaritimeRoute, SatelliteImageData, SimulatedSpillResult, AppSettings } from './types';
...
import { SatelliteAnalysisPanel } from './components/satellite/SatelliteAnalysisPanel';
...
export default function App() {
  ...
  const [targetRegion, setTargetRegion] = useState<'india' | 'atlantic' | 'indian-ocean' | 'arabian-sea' | 'bay-of-bengal' | null>(null);

  // Operator Configuration & Preferences State
  ...
  const handleUpdateSettings = (updater: (prev: AppSettings) => AppSettings) => {
    setSettings((prev) => {
      ...
      localStorage.setItem('maris_settings', JSON.stringify(next));

      // Immediate camera transition if default region is updated
      if (next.general.defaultRegion !== prev.general.defaultRegion) {
        if (next.general.defaultRegion === 'Indian Ocean') {
          setTargetRegion('indian-ocean');
        } else if (next.general.defaultRegion === 'Arabian Sea') {
          setTargetRegion('arabian-sea');
        } else if (next.general.defaultRegion === 'Bay of Bengal') {
          setTargetRegion('bay-of-bengal');
        }
      }
  ...
  const displayClasses = useMemo(() => {
    return [
      !settings.display.darkMode ? 'maris-tactical-light' : '',
      !settings.display.glassEffects ? 'maris-no-glass' : '',
      !settings.display.purpleGlow ? 'maris-no-glow' : '',
      !settings.display.animations ? 'maris-no-animations' : '',
    ]
      .filter(Boolean)
      .join(' ');
  }, [settings.display]);

  // Satellite Image Analysis state
  const [isSatelliteModalOpen, setIsSatelliteModalOpen] = useState<boolean>(false);
  const [connectedSatelliteData, setConnectedSatelliteData] = useState<{
  ...
  // Connect Satellite SAR Image directly to 3D Earth Globe & MARIS Investigation
  const handleConnectSatelliteToGlobe = (data: {
    image: SatelliteImageData;
    result: SimulatedSpillResult;
    startInvestigation: boolean;
  }) => {
    setConnectedSatelliteData({
      image: data.image,
      result: data.result,
    });

    // Link directly to the active spill in Bay of Bengal (or primary incident)
    const targetIncident =
      incidents.find((inc) => inc.id === 'SPILL-042') ||
      incidents.find((inc) => inc.severity === 'critical') ||
      incidents[0];
    ...
    if (data.startInvestigation) {
      simulation.start();
      showToast(
        `IMAGE → GLOBE: ${data.image.name} linked // Investigation initiated for ${targetIncident.id}`,
        'success'
      );
    } else {
      showToast(
        `IMAGE → GLOBE: Visualizing ${data.image.name} slick on 3D Earth (${data.result.areaKm2} km²)`,
        'info'
      );
    }
  };
  ...
  return (
    <div
      id="maris-app-root"
      className={`flex flex-col h-screen w-screen bg-[#050507] text-[#F2EDF7] overflow-hidden select-none font-sans relative ${displayClasses}`}
    >
      {/* Top Header */}
            ...
            <div className="flex-1 flex flex-col min-h-0 w-full h-full relative overflow-hidden">
              ...
              <div className="absolute inset-0 w-full h-full z-0 overflow-hidden">
                <CenterVisualization
                  ...
                  mapProjection={settings.general.mapProjection}
                  ...
                  onOpenSatelliteAnalysis={() => setIsSatelliteModalOpen(true)}
                  connectedSatelliteData={connectedSatelliteData}
                />
              </div>
              ...
              <div className="relative z-10 flex flex-col h-full w-full pointer-events-none overflow-hidden justify-between p-2 sm:p-3 lg:p-4 gap-2.5">
                ...
                <div className="flex-1 flex items-stretch justify-between min-h-0 gap-3 pointer-events-none">
                  ...
                  <div className="flex-1 pointer-events-none flex flex-col items-center justify-between min-w-0 h-full">
                    ...
                    <div className="pointer-events-auto pt-0.5 z-30 max-w-full flex justify-center">
                      <GlobeControlToolbar
                        activeLayer={activeLayer}
                        onChangeLayer={(layer) => {
