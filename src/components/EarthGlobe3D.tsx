...
export function getIncidentLabelOffset(id: string): { x: number; y: number } {
  if (id === 'SPILL-IND-043') return { x: 30, y: -20 };
  if (id === 'SPILL-IND-044') return { x: 28, y: -14 };
...
export const EarthGlobe3D: React.FC<EarthGlobe3DProps> = ({
  ...
  useEffect(() => {
    ...
    } else if (targetRegion === 'indian-ocean') {
      startCameraFlight(latLngToVector3(2.0, 78.0, 5.4), 1400);
    } else if (targetRegion === 'arabian-sea') {
      startCameraFlight(latLngToVector3(17.5, 65.0, 4.3), 1400);
    } else if (targetRegion === 'bay-of-bengal') {
      startCameraFlight(latLngToVector3(15.0, 88.0, 4.3), 1400);
    }
  }, [targetRegion, startCameraFlight]);

  useEffect(() => {
    if (mapProjection === '2D Mercator') {
      // Direct orthographic/equatorial alignment for Mercator simulation view
      startCameraFlight(latLngToVector3(5.0, 75.0, 6.2), 1200);
    } else if (mapProjection === '3D Globe' && !targetRegion) {
      startCameraFlight(latLngToVector3(14.0, 78.0, 4.8), 1200);
    }
  }, [mapProjection, startCameraFlight]);
  ...
  useEffect(() => {
    ...
    incidents.forEach((incident) => {
      ...
      const labelOffsetX = offset.x;
      const labelOffsetY = offset.y;
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center overflow-hidden select-none">
      ...
      <div
        ...
        className="pointer-events-none z-30 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[rgba(12,9,18,0.92)] border border-[rgba(245,158,11,0.4)] shadow-[0_4px_16px_rgba(0,0,0,0.8)] backdrop-blur-[12px] text-[9.5px] font-mono whitespace-nowrap"
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[#F59E0B]" />
        <span className="text-[#FBBF24] font-semibold">OIL SPILL</span>
        <span className="text-white font-medium">42.8 km²</span>
        <span className="text-[#81758F] font-mono">• 92% CONF</span>
      </div>

      <div
        ref={simOriginBadgeRef}
        style={{ display: 'none', position: 'absolute', top: 0, left: 0 }}
        className="pointer-events-none z-30 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[rgba(16,10,26,0.92)] border border-[rgba(192,132,252,0.4)] shadow-[0_4px_16px_rgba(0,0,0,0.8)] backdrop-blur-[12px] text-[9.5px] font-mono whitespace-nowrap"
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[#C084FC]" />
        <span className="text-[#D6A7FF] font-semibold">PROBABLE ORIGIN</span>
        <span className="text-white font-mono">10:24 UTC</span>
        <span className="text-[#81758F] font-mono">• ±12 km</span>
      </div>

      <div
        ref={simNordicBadgeRef}
        style={{ display: 'none', position: 'absolute', top: 0, left: 0 }}
        className="pointer-events-none z-30 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[rgba(8,16,24,0.94)] border border-[rgba(56,189,248,0.4)] shadow-[0_4px_16px_rgba(0,0,0,0.8)] backdrop-blur-[12px] text-[9.5px] font-mono whitespace-nowrap"
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[#34D399]" />
        <span className="text-[#38BDF8] font-semibold">NORDIC STAR</span>
        <span className="text-[#34D399] font-medium">92% MATCH</span>
        <span className="text-[#81758F] font-mono">• CPA 2.1 km</span>
      </div>

      <div
        ref={simForecastBadgeRef}
        style={{ display: 'none', position: 'absolute', top: 0, left: 0 }}
        className="pointer-events-none z-30 flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-[rgba(18,10,24,0.94)] border border-[rgba(217,70,239,0.4)] shadow-[0_4px_16px_rgba(0,0,0,0.8)] backdrop-blur-[12px] text-[9.5px] font-mono whitespace-nowrap"
      >
        <span className="w-1.5 h-1.5 rounded-full bg-[#D946EF]" />
        <span className="text-[#F0ABFC] font-semibold">+12H FORECAST: 67.4 km²</span>
