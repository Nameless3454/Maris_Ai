...
import { Menu, HelpCircle, Bell, Check, ExternalLink, ShieldAlert, AlertTriangle, Info, X, Satellite } from 'lucide-react';
...
export const Header: React.FC<HeaderProps> = ({
  ...
  const filteredNotifications = notifications.filter((n) => {
    ...
    if (!appSettings.notifications.criticalIncidents && n.severity === 'critical') return false;
    if (!appSettings.notifications.oilSpillAlerts && (n.title.toLowerCase().includes('spill') || n.title.toLowerCase().includes('slick'))) return false;
    if (!appSettings.notifications.vesselAnomalies && (n.title.toLowerCase().includes('vessel') || n.title.toLowerCase().includes('transponder') || n.title.toLowerCase().includes('ais'))) return false;
    if (!appSettings.notifications.satelliteUpdates && (n.title.toLowerCase().includes('satellite') || n.title.toLowerCase().includes('sar') || n.title.toLowerCase().includes('pass'))) return false;
    return true;
  });
    ...
    >
      ...
      <div className="flex items-center gap-3 sm:gap-4">
        ...
        <div className="hidden md:flex items-center gap-2 ml-3">
          ...
          {appSettings?.data.dataSource === 'LIVE SATELLITE FEED' ? (
            <span className="px-2 py-0.5 rounded-full text-[9px] font-mono font-bold uppercase tracking-wider bg-[rgba(16,185,129,0.2)] border border-[rgba(16,185,129,0.5)] text-[#34D399] hidden lg:inline-flex items-center gap-1.5 shadow-[0_0_10px_rgba(16,185,129,0.3)]">
              <span className="w-1.5 h-1.5 rounded-full bg-[#10B981] animate-ping" />
              LIVE SATELLITE FEED // COPERNICUS
            </span>
          ) : (
      ...
      <div className="flex items-center space-x-2.5 sm:space-x-4 relative">
        {/* SATELLITE IMAGE ANALYSIS Primary Action Button */}
        {onOpenSatelliteAnalysis && (
          <button
            id="header-satellite-analysis-btn"
            onClick={onOpenSatelliteAnalysis}
            aria-label="Upload and analyze satellite image"
            title="Upload and analyze SAR/EO satellite imagery for spill detection"
            className="px-2.5 sm:px-3 py-1.5 rounded-xl bg-gradient-to-r from-[rgba(0,240,255,0.22)] to-[rgba(176,38,255,0.28)] hover:from-[rgba(0,240,255,0.38)] hover:to-[rgba(176,38,255,0.45)] border border-[rgba(0,240,255,0.6)] text-[#E9D5FF] hover:text-white text-xs font-mono font-bold tracking-wider flex items-center gap-2 shadow-[0_0_15px_rgba(0,240,255,0.3)] transition-all cursor-pointer group"
          >
            <Satellite className="w-3.5 h-3.5 text-[#00F0FF] group-hover:scale-110 transition-transform animate-pulse" />
            <span className="hidden md:inline">SATELLITE IMAGE ANALYSIS</span>
            <span className="md:hidden">SATELLITE</span>
          </button>
        )}
            ...
            >
              ...
              <div className="max-h-80 overflow-y-auto divide-y divide-[rgba(255,255,255,0.04)]">
                {filteredNotifications.length === 0 ? (
                  <div className="p-6 text-center text-xs text-[#81758F] font-mono">
                    No notifications match current alert filters
                  </div>
                ) : (
